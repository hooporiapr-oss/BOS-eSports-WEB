...
router.get('/leaderboard', (req, res) => {
  ...
  // If logged in, include their rank. If not, return null.
  const userId = req.session.user ? req.session.user.id : null;
  const rank = userId ? getUserRank(userId, gameFilter, timeFilter) : null;
  res.json({ leaderboard: data, yourRank: rank, userId: userId });
});
// ─── My Scores ───
