...
const { addScore, getLeaderboard, getUserScores, getUserStats } = require('../models/store');
...
router.get('/leaderboard', auth, (req, res) => {
  const { game, tier, limit } = req.query;
  const data = getLeaderboard(game || 'strobe', tier ? parseInt(tier) : null, parseInt(limit) || 20);
...
router.get('/dashboard', auth, (req, res) => {
  const stats = getUserStats(req.session.user.id);
  const topScores = getLeaderboard('strobe', null, 10);
  res.json({
