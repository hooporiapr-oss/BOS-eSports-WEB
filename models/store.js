...
function getLeaderboard(game, tier, limit = 20, timeFilter = 'all') {
  let scores = getScores();

  // Game filter ('all' returns all games)
  if (game && game !== 'all') {
  ...
  // Tier filter
  if (tier) {
    scores = scores.filter(s => s.tier === tier);
  }
  ...
  if (timeFilter && timeFilter !== 'all') {
    ...
    if (cutoff) {
      scores = scores.filter(s => new Date(s.createdAt) >= cutoff);
    }
  }

  // For global leaderboard, keep only each player's best score per game
  // then sort by score descending
  scores = scores.sort((a, b) => b.score - a.score).slice(0, limit);
  return scores;
}

function getGlobalLeaderboard(limit = 50, timeFilter = 'all') {
  let scores = getScores();
...
function getUserRank(userId, game, timeFilter = 'all') {
  const lb = game === 'all'
    ? getGlobalLeaderboard(1000, timeFilter)
    : getLeaderboard(game, null, 1000, timeFilter);
  const idx = lb.findIndex(s => s.userId === userId);
...
module.exports = {
  ...
  addScore, getLeaderboard, getGlobalLeaderboard, getUserRank, getUserScores, getUserStats,
};